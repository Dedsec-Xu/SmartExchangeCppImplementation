...
def SEMaskResNet18(**kwargs):
    ...
    return SEMaskResNet(BasicBlock, [2,2,2,2])

def SEMaskResNet34(**kwargs):
    global THRESHOLD
    THRESHOLD = kwargs.pop('threshold', 4e-3)
    print('\nThreshold set to {:2.1e}\n'.format(THRESHOLD))
    return SEMaskResNet(BasicBlock, [3,4,6,3])

def SEMaskResNet50(**kwargs):
    global THRESHOLD
    THRESHOLD = kwargs.pop('threshold', 4e-3)
    print('\nThreshold set to {:2.1e}\n'.format(THRESHOLD))
    return SEMaskResNet(Bottleneck, [3,4,6,3])

def SEMaskResNet101(**kwargs):
    global THRESHOLD
    THRESHOLD = kwargs.pop('threshold', 4e-3)
    print('\nThreshold set to {:2.1e}\n'.format(THRESHOLD))
    return SEMaskResNet(Bottleneck, [3,4,23,3])

def SEMaskResNet152(**kwargs):
    global THRESHOLD
    THRESHOLD = kwargs.pop('threshold', 4e-3)
    print('\nThreshold set to {:2.1e}\n'.format(THRESHOLD))
    return SEMaskResNet(Bottleneck, [3,8,36,3])
